...
/* VERSIÓN INICIAL de advanceRoad */
static size_t advanceRoad(const unsigned char *currentRoad, unsigned char *nextRoad, size_t cellCount) {
    ...
    for (size_t j = 0; j < cellCount; ++j) {
        size_t left  = (j == 0) ? (cellCount - 1) : (j - 1);
        size_t right = (j == cellCount - 1) ? 0 : (j + 1);

        unsigned char self_has_car = currentRoad[j];
        unsigned char left_has_car = currentRoad[left];
        unsigned char right_is_free = (currentRoad[right] == 0);

        // Caso 1: Un coche entra desde la izquierda
        if (left_has_car && self_has_car == 0) {
            nextRoad[j] = 1u;
            moved_ull += 1ULL;
        } 
        // Caso 2: Tenía un coche pero NO pudo moverse (bloqueado por el de la derecha)
        else if (self_has_car && !right_is_free) {
            nextRoad[j] = 1u;
        }
        // Caso 3: La celda queda vacía (el coche se movió o ya estaba vacía)
        else {
            nextRoad[j] = 0u;
        }
    }
