...
int main(int argc, char *argv[]) {
    ...
    // Gather current state for verification on small inputs only
    if (totalCells <= 100) {
        MPI_Gatherv(&nextRoad[1], localCount, MPI_UNSIGNED_CHAR,
                    globalRoad, counts, displs, MPI_UNSIGNED_CHAR,
                    0, MPI_COMM_WORLD);
        if (rank == 0) {
            printf("Step %zu: %zu cars moved | Road: ", step, totalMoved);
            printRoad(globalRoad, totalCells);
        }
    }
